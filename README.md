...
installeren.
Maak een gratis GitHub-account (indien je die nog niet hebt) en maak een
nieuwe, publieke of privé repository aan.
Upload de inhoud van deze map naar die repository (via de "Add file" →
"Upload files" knop op github.com, of met `git push`).
Ga naar het tabblad Actions van je repository. De workflow "Build mod"
start automatisch.
Wacht tot de run groen is (klaar). Klik erop, en onderaan bij
